...
Документ фиксирует, какие зависимости разрешены, какие legacy-источники запрещены, где могут жить исключения и как добавлять новые платёжные интеграции.
...
5. Legacy-код сайта можно читать только для миграции или диагностики; новая логика туда не добавляется.
...
## Запрещённые legacy-источники

В runtime-коде модуля запрещено использовать:

- legacy IBLOCK 13/14 для оплат, подписки или доступа;
- `payments/check.php`;
- `api/tinkoff/*`;
- таблицу `b_payments`;
- старые обработчики сайта как источник истины по оплате;
- Sale/заказы Bitrix для доменной модели платежей модуля.

Разрешённые исключения:

- read-only анализ legacy-данных при миграции;
- admin utility для миграции документов из IBLOCK;
- диагностические упоминания legacy путей в документации/аудите.
...
- legacy payment endpoints/tables.
...
- обращаться к legacy payment sources;
